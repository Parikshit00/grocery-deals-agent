...
class NettoRecipe:
    ...
    async def fetch(self, zip_code: str, max_pages: int = 80) -> ProspektPages:
        ...
        target = await self._find_handzettel(week)
        if not target:
        ...
        pages = await publitas_best_pages(captured)
        log.info("netto.captured", pages=len(pages))
        return ProspektPages("netto", "national", vf, vt, pages)

    async def _find_handzettel(self, week: int) -> str | None:
